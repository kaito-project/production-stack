...
| `EnvoyFilter` | `networking.istio.io/v1alpha3` | Istio | `charts/modelharness` renders the per-namespace `bbr-ext-proc` filter (injects BBR's ext_proc into the namespace Gateway's HCM, scoped via `workloadSelector`), the `model-not-found-direct` filter (OpenAI 404 `direct_response` catch-all for unknown models), and the `gateway-filter-outage-local-reply` filter (maps fail-closed BBR/ext_authz outages onto the unified error envelope). |

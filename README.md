...
1. **Client → Istio Gateway.** Client sends `GET /v1/models` (or `GET /v1/models/{id}`) to `<namespace>.gw.example.com`.
2. **ext_authz runs first.** `ext_authz` is an HTTP filter, so it executes before route selection — an unauthenticated caller cannot enumerate a namespace's models.
3. **Gateway → status reporter.** Two routes rendered by `charts/modelharness` into the same `model-not-found-direct` `EnvoyFilter`, ahead of the catch-all routes, forward the request to the cluster-wide `productionstack-status-reporter` Service and stamp `X-Kaito-Gateway-Namespace` with `OVERWRITE_IF_EXISTS_OR_ADD`. The namespace is taken from that header rather than `Host`, which is caller-supplied and therefore forgeable.
4. **Reporter → response.** The reporter answers from an informer cache of the `InferenceSet`s in that namespace, returning the OpenAI shape (`id`, `object`, `created`, `owned_by`). `id` is the `InferenceSet` name — exactly the value `X-Gateway-Model-Name` matches — so a discovered model can be invoked as-is. The listing is a **registry**: a model appears as soon as it is declared, regardless of whether its pods are serving yet.

The routes must precede the catch-all routes: a bodyless `GET` gives BBR nothing to extract, so without them the request would fall through to the unconditional `400 invalid_request_body`.
...
| `InferenceSet` | `kaito.sh/v1beta1` | KAITO | Declares one model deployment; KAITO renders inference pods. |

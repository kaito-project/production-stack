...
| `bbr.podAntiAffinity.enabled`                    | `true`                                                                        | Render a pod-anti-affinity rule so no two BBR replicas are co-scheduled on the same `topologyKey` domain.                       |
| `bbr.podAntiAffinity.type`                       | `soft`                                                                        | `soft` ⇒ `preferredDuringScheduling…` (best-effort spread, still schedules on a single-node cluster); `hard` ⇒ `requiredDuringScheduling…` (a replica stays Pending until a distinct node is free). |
...
| `bbr.healthCheck.enabled`                        | `true`                                                                        | Render gRPC liveness/readiness probes against BBR's `grpc.health.v1.Health` service on `healthCheckPort`. The readiness result drives the pod Ready condition → Istio EDS endpoint set → ext_proc cluster, so only healthy replicas receive traffic. |
| `bbr.healthCheck.{liveness,readiness}.*`         | `initialDelaySeconds:5, periodSeconds:10, timeoutSeconds:5/3, failureThreshold:3` | Standard probe timing knobs.                                                                                              |

...
| `bbr.image.{registry,repository,tag,pullPolicy}` | released registry `registry.k8s.io`, tag `v1.5.0`                             | Container image coordinates. Pinned to the stable released registry (the k8s staging registry garbage-collects tags); upstream chart defaults to the staging registry / `main` tag. |

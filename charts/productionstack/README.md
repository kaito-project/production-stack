...
| `productionstack-status-reporter` | Read-only control-plane reporter that publishes the end-to-end error-handling reason catalogue as `Event`s in `kube-system` (leader-elected), **and** serves the OpenAI model-discovery endpoints `GET /v1/models` / `GET /v1/models/{id}` behind a cluster-singleton `Service` whose name is pinned to `productionstack-status-reporter`. Each workload namespace's Gateway routes discovery traffic to that Service, so if you move this subchart or change `modelsAPI.port`, set the matching `modelsAPI.serviceNamespace` / `.servicePort` on every `modelharness` release. Unlike the event loop, the discovery endpoint runs on every replica. | `productionstack-status-reporter.enabled` (default `true`) | `kube-system` |

...
| `replicas`                | required | `1`                                                                  | InferenceSet replicas. Also wired to `scaledobject.kaito.sh/min-replicas`. When `enableScaling=true` this is the lower bound only: `spec.replicas` is written on install and thereafter owned by KEDA, so upgrades do not reset a scaled-out deployment. |

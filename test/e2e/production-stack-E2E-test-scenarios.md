...
Basic sanity checks that the test framework is wired correctly and the Gateway is reachable before heavier tests run. Backed by the reusable `test/e2e/scenarios` library (`gpu_smoke_test.go` / `CaseGPUSmoke`), against its own small dedicated deployment separate from the GPU-mocker Infra/Routing case below.
...
* Correct model name for gemma — Send `POST /v1/chat/completions` with `{"model": "routing-gemma", ...}`. Verify `response.model == "routing-gemma"`. Proves the full BBR → HTTPRoute → EPP → pod chain works for the first model.
* Correct model name for ministral — Same validation for the second model; catches per-pool misrouting.
* Cross-model isolation (serial) — Send N requests for each model sequentially. Scrape `vllm:request_success_total{model_name}` from every shadow pod before and after. Verify that gemma pods' counters only incremented for gemma requests, and ministral pods' counters only incremented for ministral requests. No cross-pool contamination.
* Cross-model isolation (concurrent) — Launch **interleaved concurrent** traffic: 20 in-flight gemma + 20 in-flight ministral requests at the same time. Verify per-pod `vllm:request_success_total{model_name}` still shows zero cross-contamination. BBR and EPP are two chained ext_proc filters; serial tests cannot expose header-state leakage between concurrent requests within the same Envoy worker.
...
* EPP routing success (metrics) — After the above runs, verify `inference_extension_scheduler_attempts_total{status="success"}` increased by the total requests sent and `{status="failure"}` did not change. Verify `inference_objective_request_total{model_name="routing-gemma"}` and `{model_name="routing-ministral"}` match the per-model counts. Proves EPP actively scheduled each request rather than falling through to a default route.

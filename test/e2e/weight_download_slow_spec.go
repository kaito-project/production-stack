...
var _ = Describe("Weight Download Slow Reporter",
	Ordered, utils.GinkgoLabelStatusReporter, func() {
		...
		It("does not emit inferencesetWeightDownloadSlow for a healthy deployment", func() {
			baseline, err := utils.ReporterEventBaseline(ctx, "inferencesetWeightDownloadSlow", caseNS)
			Expect(err).NotTo(HaveOccurred())
			Expect(utils.EnsureNoReporterEventSince(ctx, "inferencesetWeightDownloadSlow", caseNS, baseline, quietDwell)).
				To(Succeed(),

...
| `type` | Always `Warning` — the reporter only publishes problems; a healthy state is the absence of a `Warning`, not a positive event. Every reason string ends in `*NotReady` / `*Failed` / `*Missing` / `*Slow`. |
...
A `Warning` event is emitted while its reason is active; a healthy state is the absence of a `Warning`, not a positive event. Repeats while the state is unchanged are suppressed by the standard client-go `EventRecorder` aggregation behaviour; the `count` and `lastTimestamp` fields on the existing event are bumped instead.
...
| --- | --- | --- | --- | --- | --- |
| Cluster | `clusterCRDMissing` | Warning | Required CRD absent. The CRD set is derived from what `charts/productionstack`, `charts/modelharness`, and `charts/modeldeployment` actually render or reference (including RBAC grants for runtime informers): Gateway API (`Gateway`, `HTTPRoute`); GAIE (`InferencePool`); KAITO (`InferenceSet`, `Workspace`, `APIKey`); Istio (`EnvoyFilter`, `AuthorizationPolicy`); KEDA (`ScaledObject`, `ClusterTriggerAuthentication`) | API discovery by the reporter (periodic); each affected component additionally poll-then-startup-timeout-exits and is restarted by Kubernetes until the CRD appears | the missing `CustomResourceDefinition` (cluster-scoped) |
...
| Modelharness | `modelharnessGatewayDataPlaneNotReady` | Warning | The `Gateway` is `Programmed=True` (istiod created the auto-provisioned Deployment/Service and assigned an address) but the backing Envoy data-plane pod is not running — typically **Pending/unschedulable** (insufficient CPU/GPU/quota, no matching node) or otherwise NotReady. istiod sets `Programmed=True` **without** waiting for the pod to schedule, so every `Gateway` / `HTTPRoute` / `InferencePool` condition can be green while no traffic can flow; the reporter probes the data-plane pod directly to close that control-plane-green / data-plane-down gap. | list the Deployment(s) labelled `gateway.networking.k8s.io/gateway-name=<gateway>` and assert a pod is Ready (a Pending pod has no Ready condition, so pod `Phase`/`PodScheduled` is also inspected); debounced via a 5-minute grace window (a fresh pod may wait for a node to scale up) | the workload `Namespace` (the `Gateway` name and the scheduling cause are identified in `message`) |
...
| Modelharness | `modelharnessEnvoyFilterMissing` | Warning | one or more of the per-namespace dataplane `EnvoyFilter`s that `charts/modelharness` provisions on the Gateway HCM is absent (chart not fully installed or drift): the unconditional `bbr-ext-proc`, `model-not-found-direct`, `gateway-filter-outage-local-reply`, and — when API-key auth is enabled (an `APIKey` CR exists) — `apikey-ext-authz` | reconcile-time existence check by the reporter: list `EnvoyFilter`s in the namespace and assert each required name is present (`EnvoyFilter` carries no status conditions, so absence is the only reliable signal); debounced via the startup grace window | the workload `Namespace` (the missing `EnvoyFilter` name(s) are identified in `message`) |
| Modelharness | `modelharnessNetworkPolicyMissing` | Warning | the per-namespace `CiliumNetworkPolicy` `inference-pods-ingress` that `charts/modelharness` renders (guarded by `networkPolicy.enabled`, default on) is absent — chart not fully installed / drift, or the Cilium dataplane never compiled it — leaving inference pods without East-West ingress isolation | reconcile-time existence check by the reporter: list `CiliumNetworkPolicy`s in the namespace and assert `inference-pods-ingress` is present (the policy carries no readiness condition, so absence is the only reliable signal); debounced via the startup grace window | the workload `Namespace` (the missing `CiliumNetworkPolicy` name is identified in `message`) |
| Modeldeployment | `inferencesetInfraProvisioningFailed` | Warning | GPU node cannot be provisioned — quota exceeded, instance type/SKU unavailable, zone capacity, subscription not registered — covers both initial provisioning and runtime re-provisioning after a previously Ready node is reclaimed or fails | the child `Workspace`'s karpenter `NodeClaim.status.conditions[Launched]=False` (KAITO does **not** put a terminal reason on the `Workspace` — it stays `NodeClaimReady=False`/`NodeClaimNotReady`, indistinguishable from in-progress — so the reporter reads the underlying `NodeClaim`, whose reason/message carries the cloud cause). A non-retryable reason (`SKUNotAvailable`, or "all requested instance types were unavailable during launch") is surfaced immediately; retryable quota/capacity reasons are debounced ~10m since karpenter auto-retries with an alternative SKU/zone | the workload `Namespace` (the owning `InferenceSet` and failing `NodeClaim` are identified in `message`) |
| Modeldeployment | `inferencesetModelPodsNotReady` | Warning | Same reason regardless of whether the pod has ever reached Ready. Install-time: `ImagePullBackOff` on base image, model-weights pull failure, `InsufficientGPU`, PVC bind failure. Runtime: `OOMKilled`, `CrashLoopBackOff`, readiness-probe regression, eviction. | Pod `status.containerStatuses[*].state` + `restartCount` — a terminal container failure on the init model-weights-downloader or main vLLM container, detected via the pod's `Ready` condition; a merely still-starting pod within KAITO's ~30 min StartupProbe budget is not surfaced here | the workload `Namespace` (the owning `InferenceSet` name and the failing `Pod` are identified in `message`) |
...
The reporter emits **every** active reason for a layer as its own event — there is no single-primary selection or priority collapse. Concurrent faults must each surface independently so one problem is never hidden behind another, and the absence of a `Warning` is the only "healthy" signal. Within a layer, findings are grouped by subject (the workload `Namespace` for the cluster and modelharness layers, the owning `InferenceSet` for the modeldeployment layer) and de-duplicated so each reason is reported at most once per subject per reconcile. Cross-layer dependencies are **not** re-classified into the harness or modeldeployment layer; a cluster-layer outage stays a cluster-layer event in `kube-system` and operators consult cluster-layer events first (see Story 2 / Story 3). §1.4 below adds a narrow cross-layer **suppression** rule for the small set of downstream reasons that are definitionally dependent on an upstream cluster reason.
...
| `clusterKaitoControllerNotReady` | `inferencesetInfraProvisioningFailed`, `inferencesetModelPodsNotReady` | The KAITO controller creates the child `Workspace`s and their karpenter `NodeClaim`s; without it no `NodeClaim` is created, so a `NodeClaim Launched=False` (infra failed) vs no-NodeClaim-yet (infra not attempted) and "pod not scheduled" vs "pod not yet observed" cannot be distinguished. `inferencesetEPPNotReady` and `inferencesetRouteNotReady` are **not** gated here — EPP `Deployment` status is reconciled by kube-controller-manager; `HTTPRoute` / `InferencePool` status is written by Gateway API controllers (Istio) — both independent of KAITO. |
...
In addition to the cluster-wide rows above, one **namespace-scoped** row suppresses a modeldeployment reason from an active modelharness reason **within the same workload namespace only** (a namespace `Gateway` is per-namespace, so the gating must not leak across namespaces):

| Active upstream `modelharness*` reason (per namespace) | Suppressed downstream reason (same namespace) | Why this is a definitional dependency |
| --- | --- | --- |
| `modelharnessGatewayClassMissing`, `modelharnessGatewayProgrammingFailed`, `modelharnessGatewayDataPlaneNotReady` | `inferencesetRouteNotReady` | Every `HTTPRoute` in the namespace attaches to that one `Gateway` via `parentRefs`. When the `Gateway` is not accepted / not programmed, each attached `HTTPRoute` necessarily reports `status.parents[].conditions[Accepted]=False` (`NoMatchingParent`); the route errors are a mechanical consequence of the Gateway problem, so they are gated while the Gateway problem is surfaced once for the namespace. When the `Gateway` is Programmed but its data-plane pod is not running, route readiness is likewise moot — no traffic can reach any route — so `inferencesetRouteNotReady` is gated in that namespace too. The `ResolvedRefs` / `InferencePool` dimension of `inferencesetRouteNotReady` is still surfaced when the Gateway is healthy. |

Behavior:

1. On every reconcile, the reporter evaluates upstream cluster reasons first (the cluster layer is evaluated before the harness and modeldeployment layers). For each currently-active upstream reason, the reporter consults the suppression table.
...
3. When the upstream resolves, suppression lifts on the next reconcile and the downstream reason is re-evaluated and emitted if still applicable.
...
- **Mirror sub-conditions as new `InferenceSet` condition Types (`InferenceSetInfraReady`, `InferenceSetModelPodsReady`).** Rejected: the signals already exist on owned resources the reporter reads directly — the karpenter `NodeClaim.status.conditions[Launched]=False` (→ `inferencesetInfraProvisioningFailed`; the owned `Workspace` deliberately keeps `NodeClaimReady=False`/`NodeClaimNotReady` on failure, so it is not a reliable terminal signal) and the model `Pod` readiness. New `InferenceSet` Types would duplicate that signal and add a CR-status contract no consumer reads — this proposal aggregates only on the `kube-system` event stream, not on `InferenceSet.status`. EPP, `HTTPRoute`, and cross-layer dependencies sit outside `Workspace` and are detected by the reporter's own watches.
...
| --- | --- | --- | --- |
| Cluster | `cluster_status_test.go` (new) | scale `body-based-routing` Deployment to zero, then back | `Warning` `clusterBBRNotReady` appears in `kube-system` within one reporter resync; clears when scaled back (the Warning stops being refreshed and ages out) |
...
| Modelharness | `harness_status_test.go` (new) | hand-edit `AuthorizationPolicy` provider name while `llm-gateway-auth` is healthy | `Warning` `modelharnessExtAuthzProviderMissing` whose `message` names the affected namespace; clears when the chart is re-applied |
...
| Modelharness | extend `network_policy_test.go` | delete the `inference-pods-ingress` `CiliumNetworkPolicy` | `Warning` `modelharnessNetworkPolicyMissing` emitted; clears when the policy is re-applied |
...
| Modeldeployment | `control_plane_error_test.go` (new) | after the InferenceSet is healthy (no Warning), scale the EPP `Deployment` to zero | transitions to `Warning` `inferencesetEPPNotReady` on the workload `Namespace` (per §1.1; the EPP `Deployment` name is identified in `message`); clears when scaled back (verifies post-install drift detection) |
| Modeldeployment | `control_plane_error_test.go` (new) | after the InferenceSet is healthy (no Warning), delete the parent namespace `Gateway` | `Warning` `inferencesetRouteNotReady` on the workload `Namespace` (per §1.1; the failing `HTTPRoute` is identified in `message`) |
